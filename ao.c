...
data d1, d2;
{
	...
	if(f1 == 0.) return(d2 == zero ? (data)1.0 : zero);
...
data d1, d2;
{
	...
	case 5:
		/* sinh */
		f = d2;
		if(f < -maxexp || f > maxexp) error("sinh D");
		f = exp(f);
		f = (f-1./f)/2.;
		goto ret;
	...
	case 6:
		/* cosh */
		f = d2;
		if(f < -maxexp || f > maxexp) error("cosh D");
		f = exp(f);
		f = (f+1./f)/2.;
		goto ret;
...
data d1, d2;
{
	if(d1!=zero && d2!=zero) return(zero);
	return(one);
}
